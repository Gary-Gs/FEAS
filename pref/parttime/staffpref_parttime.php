...
<?php
	...
	try {
			...
			//$staffid= GetSQLValueString($stid, "text");
	...
	try
	{
		...
		$query_rsOtherSettings = "SELECT * FROM ".$TABLES['allocation_settings_others']." as g WHERE type = 'PT'";
		...
		$query_rsSupervisingProject = "SELECT p1.project_id, p1.year, p1.sem, p1.staff_id, p2.title, p3.examine_year, p3.examine_sem FROM ".$TABLES['fyp_assign_part_time']." as p1 LEFT JOIN ".$TABLES['fyp']." as p2 ON p1.project_id=p2.project_id LEFT JOIN ". $TABLES['fea_projects_part_time'] . " as p3 on p1.project_id = p3.project_id WHERE p1.complete = 0 AND p1.staff_id= ? AND p3.examine_year =?  AND p3.examine_sem = ?". " ORDER BY p1.project_id";
		...
		if ($staffPrefOpened)
		{
			...
			$query_rsProject = "SELECT p3.project_id as pno, p2.staff_id as staffid, p1.title as ptitle FROM ".$TABLES['fea_projects_part_time']." as p3 LEFT JOIN ".$TABLES['fyp_assign_part_time']." as p2 ON p3.project_id = p2.project_id LEFT JOIN ".$TABLES['fyp']." as p1 ON p2.project_id = p1.project_id WHERE p2.complete = 0 and p3.examine_year = ".$examYearValue." and p3.examine_sem = ".$examSemValue." ORDER BY p3.project_id ASC ";
			
			$query_rsArea	 	= "SELECT * FROM ".$TABLES['interest_area']." where title <> '-' ORDER BY title ASC, `key` ASC";

			$query_staffProjPref	= "SELECT * FROM ".$TABLES['staff_pref_part_time']." WHERE staff_id = ? AND (prefer LIKE 'SCE%' OR prefer LIKE 'SCSE%')  AND archive =0 ORDER BY choice ASC";
			
			$query_staffAreaPref	= "SELECT * FROM ".$TABLES['staff_pref_part_time']." as sp INNER JOIN ". $TABLES['interest_area'] ." as ia ON sp.prefer= ia.key AND archive =0  WHERE staff_id = ? ORDER BY choice ASC";
...
<body>
	...
	<div id="wrapper">
		...
		<div id="header"></div>
		<div id="content">
			<h1>Staff Preference for Part Time Projects</h1>
			...
			<div class="ui-widget">
				...
				<form action="submitpref.php" method="post">
					...
					<div style="float:right; padding-top:25px;">
						...
						<a href="staffpref_parttime.php?clear=1" class="bt" title="Clear all selected preferences">Clear Changes</a>
