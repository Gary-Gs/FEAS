...
<?php
	...
	try
	{
        ...
        //$numRoom 		= $conn_db_ntu->query($query_numRoom)->fetch(); // day 1 no. of rooms 
	...
	function generateTimeSelect($id, $start, $end, $interval, $selected)
	{
		...
		$timeSelectStr = "";
		$timeSelectStr.= '<select id="'.$id.'" name="'.$id.'">';
	...
	function generateDurationSelect($id, $start, $end, $interval, $selected)
	{
		...
		$time_interval	= new DateInterval('PT'.$interval.'M');
		$durationSelectStr  = "";
		$durationSelectStr.= '<select id="'.$id.'" name="'.$id.'">';
		for ($curTime=$start_time; $curTime <= $end_time; $curTime->add($time_interval))
		{
								...
								<div id="exam_settings">
									...
									<table id="examsettings_table" border="0" width="406" style="text-align:left;">
										...
										<tr>
											...
											<td><input type="number" id="number_of_days" name="number_of_days" min="1" max="10" value="<?php echo $NO_OF_DAYS; ?>" required /><br/>
											<span id="dayErrorMsg" class = "errorMsg"></span></td>
										</tr>
									</table>
								</div>
							</td>
