...
$headers = ['Staff ID', 'Email', 'Staff Name', 'Exemption', 'Supervising Projects', "Examining Projects", "Preferred Project No.", "No. Projects Not in Proj Pref List", "Total Load After Assignment"];
$objPHPExcel->getActiveSheet()->fromArray($headers, NULL, 'A1');
$objPHPExcel->getActiveSheet()->getStyle('A1:J1')->getFont()->setBold(true);

$exempt_sem = ($otherSettings['exam_sem'] == 2) ? "exemptionS2" : "exemption";
$rowCount = 2;
foreach ($examining_staff as $staff) {
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $staff[$exempt_sem]);
	foreach ($supervising_load as $supervising) {
		if ($supervising['staff_id'] == $staff['id']) {
			if ($supervising['supervising_count'] == null || $supervising['supervising_count'] == "") {
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $supervising['supervising_count']);
			}
	...
	foreach ($examining_load as $examining) {
		if ($examining['examiner_id'] == $staff['id']) {
			if ($examining['examiner_count'] == null || $examining['examiner_count'] == "") {
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $examining['examiner_count']);
			}
		}
	}
	...
	foreach ($preferredProjects as $preferred) {
		if ($preferred['staff_id'] == $staff['id']) {
			$objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $preferred['prefer']);
		}
	}

	foreach ($notPreferred as $notPrefer) {
		if ($notPrefer['id'] == $staff['id']) {
			$objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $notPrefer['not_preferred']);
		}
	}

	$totalLoad = $objPHPExcel->getActiveSheet()->getCell('D' . $rowCount)->getValue() + $objPHPExcel->getActiveSheet()->getCell('F' . $rowCount)->getValue();
	$objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $totalLoad);
