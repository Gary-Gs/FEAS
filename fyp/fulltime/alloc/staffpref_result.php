...
if (!isset($_POST['filterProject_To']) || $_POST["filterProject_To"] == $currentYrSem) {
    ...
    // if current year sem, extract the yr and sem value
    $extractYrSemFromCurrentYrSem = preg_replace('/\D/', '', $currentYrSem);
    $extractYr = substr($extractYrSemFromCurrentYrSem, 0, 4);
    $extractSem = substr($extractYrSemFromCurrentYrSem, strlen($extractYrSemFromCurrentYrSem)-1, 1);

    $query_rsProjectPreference = "SELECT project.project_id, project.title, count(DISTINCT p4.staff_id) as total FROM ".
        " (SELECT DISTINCT(p1.project_id), p2.title FROM fea_projects as p1 LEFT JOIN fyp as p2 ON p1.project_id = p2.project_id WHERE p1.examine_year = '" . $extractYr . "' and p1.examine_sem = '" . $extractSem . "') AS project " .
        " LEFT JOIN fea_staff_pref as p4 ON project.project_id = p4.prefer GROUP BY project.project_id, project.title ORDER BY project.project_id ASC";
...
{
    $filter_proj=$_POST["filterProject_To"];
    $yrsem = trim(preg_replace('!\s+!', ' ', $filter_proj));
    $words = explode(" ", $yrsem);
    $query_rsProjectPreference = "SELECT project.project_id, project.title, p2.count as total, p2.choose_date FROM " . $TABLES['fyp'] . " as project LEFT JOIN " . $TABLES['staff_pref_count'] . " as p2 ".
        ...
        "WHERE p2.choose_date  = '" .$words[0]. " ".$words[1]. " " .$words[2]. " " .$words[3]. "' GROUP BY project.project_id, project.title " .
        ...
        if (($indexFrom - $indexTo) <= 3) {
            ...
            for ($i = $indexFrom; $i >= $indexTo && $i > -1; $i--) {
                ...
                if ((($_POST["filter_To"] == $currentYrSem) && ($chooseDate == $currentYrSem)) || ((!isset($_POST["filter_To"])) && ($chooseDate == $currentYrSem))) {
                    ...
                    $area_yrsem = trim(preg_replace('!\s+!', ' ', $chooseDate));
                    $areawords = explode(" ", $area_yrsem);
                    $query_rsInterestArea = "SELECT p1.key, p1.title, p2.count as total, p2.choose_date FROM " . $TABLES['interest_area'] . " as p1 LEFT JOIN " . $TABLES['staff_pref_count'] . " as p2 ".
                        "ON p1.key = p2.prefer ".
                        "WHERE p2.choose_date = '" .$areawords[0] . " ". $areawords[1] . " " . $areawords[2] . " " .  $areawords[3] . "'".
                        "GROUP BY p1.key, p1.title ".
                        "ORDER BY p1.key ASC";
...
// Get previous sem results, exclude current semester
        $filter_area = $_POST["filter_To"];
        $yrsem = trim(preg_replace('!\s+!', ' ', $filter_area));
        $words = explode(" ", $yrsem);

        $sem = str_split($words[1], 2); //[0] = 18 [1] = 19
        //echo $sem[0];
        //sem1 jul-dec
        if ($words[3] == '1') {
            // echo $words[3];
            $query_rsTotalInterestArea = "SELECT count(DISTINCT p.staff_id) as total FROM "
                . $TABLES["staff_pref"] . " as p where (choose_time LIKE '%$sem[0]-07%'
             OR choose_time LIKE '%$sem[0]-08%' OR choose_time LIKE '%$sem[0]-09%'
             OR choose_time LIKE '%$sem[0]-10%' OR choose_time LIKE '%$sem[0]-11%'
             OR choose_time LIKE '%$sem[0]-12%') AND prefer LIKE 'k%'";
        } //sem2 jan-jun
        else {
            $query_rsTotalInterestArea = "SELECT count(DISTINCT p.staff_id) as total FROM "
                . $TABLES["staff_pref"] . " as p where (choose_time LIKE '%$sem[1]-01%'
             OR choose_time LIKE '%$sem[1]-02%' OR choose_time LIKE '%$sem[1]-03%'
             OR choose_time LIKE '%$sem[1]-04%' OR choose_time LIKE '%$sem[1]-05%'
             OR choose_time LIKE '%$sem[1]-06%') AND prefer LIKE 'k%'";
        }
    }
}
else
{
    //if given a range, set the pie chart to the latest project record only
    $query_rsTotalInterestArea = "SELECT COUNT(DISTINCT p2.staff_id) as total FROM " . $TABLES['interest_area'] . " as p1 LEFT JOIN " . $TABLES['staff_pref'] . " as p2 " .
        "ON p1.key = p2.prefer " .
        "WHERE p2.archive = 0 or p2.archive IS NULL ";
}
...
//pie chart for project preference
if (!isset($_POST['filterProject_To']) || $_POST["filterProject_To"] == $currentYrSem) {
    ...
    if ($words[3] == '1')
    {
        $query_rsTotalProjectPreference = "SELECT count(DISTINCT p.staff_id) as total FROM "
            . $TABLES["staff_pref"] . " as p where (choose_time LIKE '%$sem[0]-07%'
             OR choose_time LIKE '%$sem[0]-08%' OR choose_time LIKE '%$sem[0]-09%'
             OR choose_time LIKE '%$sem[0]-10%' OR choose_time LIKE '%$sem[0]-11%'
             OR choose_time LIKE '%$sem[0]-12%') AND prefer LIKE 'SCSE%' OR 'SCE%' ";
    }
    //sem2 jan-jun
    else {
        $query_rsTotalProjectPreference = "SELECT count(DISTINCT p.staff_id) as total FROM "
            . $TABLES["staff_pref"] . " as p where (choose_time LIKE '%$sem[1]-01%'
             OR choose_time LIKE '%$sem[1]-02%' OR choose_time LIKE '%$sem[1]-03%'
             OR choose_time LIKE '%$sem[1]-04%' OR choose_time LIKE '%$sem[1]-05%'
             OR choose_time LIKE '%$sem[1]-06%') AND prefer LIKE 'SCSE%' OR 'SCE%'";
    }
    /*
        $query_rsTotalProjectPreference = "SELECT sum(p2.count) as total FROM "
            . $TABLES['fyp'] . " as project LEFT JOIN " . $TABLES['staff_pref_count'] . " as p2 " .
            "ON project.project_id = p2.prefer " .
                " LEFT JOIN " . $TABLES['allocation_settings_others'] . " as p3 " .
                " ON p3.exam_year = " . $words[1] . " AND p3.exam_sem = " . $words[3] . " WHERE p3.type = 'FT'" .
                " AND p2.choose_date  = '" .$words[0]. " ". $words[1] . " " . $words[2] . " " . $words[3] . "' ";
    */
}
