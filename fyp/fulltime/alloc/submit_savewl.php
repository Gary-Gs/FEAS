...
<?php
	...
	foreach ($rsStaff as $curStaff)
	{
		...
		if(isset($_REQUEST['index_'.$staffid]))
		{
			...
			$name = GetSQLValueString(strtolower(trim($_REQUEST['name_' .$staffid])),"text");
			$name2 = GetSQLValueString(strtolower(trim($_REQUEST['name2_' .$staffid])),"text");
			...
			if (strtolower(trim($_REQUEST['name_' .$staffid])) != $curStaff['name'] || strtolower(trim($_REQUEST['name2_' .$staffid])) != $curStaff['name2'] || $canExamine != $curStaff['examine'] || $exemption != $curStaff['exemption']) {
	...
	if (isset($_REQUEST['newEmail'])) {
		foreach ($_REQUEST['newEmail'] as $newRecord) {
			...
			$newName = GetSQLValueString(strtolower(trim($_REQUEST['newName'][$c])),"text");
			$newName2 = GetSQLValueString(strtolower(trim($_REQUEST['newName2'][$c])),"text");
