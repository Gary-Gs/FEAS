...
<?php
	...
	{
		//updates the first date
			$firstAllocDate = $_REQUEST['alloc_date'];
	
			$stmt = $conn_db_ntu->prepare("INSERT INTO ". $TABLES['allocation_settings_general'] . " (`id`, `alloc_date`) VALUES (1 , ?) ON DUPLICATE KEY UPDATE `alloc_date`= VALUES(`alloc_date`)");
			$stmt->bindParam(1, $firstAllocDate);
			$stmt->execute();
		
	}
	if(isset($_REQUEST['alloc_days']))
	{
		//updates the next 1 or 2 days other than first date
		for($i=1;$i<sizeof($_REQUEST['alloc_days']);$i++){
			$nextAllocDate = $_REQUEST['alloc_days'][$i];
			...
			$stmt->bindParam(1, $id);
			$stmt->bindParam(2, $nextAllocDate);
			$stmt->execute();
	...
	while(isset($_REQUEST['room_'.$i])) {
		...
		if(empty($check))
			...
			if ($check != "NULL")
			{       
                                // double check if the records exist in the database aldr // if exist then do not need to insert again
    ...
    // update (id, roomName2) in the event have // scenario 2, what if day 1 0 entries, then do checking, then do insert instead of update. Repeat for Day 3
				...
				{    
					...
					{   
					    ...
					    $stmt->execute();
                                        ...
                                        {
                                            // re-insert the values into database // assuming that id exist then can update
