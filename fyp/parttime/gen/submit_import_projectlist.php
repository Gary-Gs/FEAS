...
if(isset($_FILES["file"]["name"])){	
	...
	if($inputFileType == "xlsx" || $inputFileType == "xls" || $inputFileType == "csv" ){
		if(move_uploaded_file($_FILES["file"]["tmp_name"], $target_file )){
			...
			//by right wont hit if open from other directory instead of server
...
function HandleExcelData($objPHPExcel){
	...
	for($row = 2; $row <= $highestRow ; $row++ )
	{
		...
		try {
			$query_duplicateProject = "SELECT * FROM .". $TABLES['fea_projects_part_time']." WHERE `project_id` = ?"; 
			$stmt = $conn_db_ntu->prepare($query_duplicateProject);
			$stmt->bindParam(1, $proj_id);
			$stmt->execute();
			$fea_projects = $stmt->fetchAll();

			$query_duplicateFypAssign = "SELECT * FROM .".$TABLES['fyp_assign_part_time'] ." WHERE `project_id` = ?"; 
		...
		if($proj_id == "") {
			continue;
				...
				//remove those with student statuses stated below
