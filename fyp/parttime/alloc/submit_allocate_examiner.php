...
$WORKLOAD_TOTALPROJECTS        = 0;
	...
	$query_rsOtherSettings= "SELECT * FROM ".$TABLES['allocation_settings_others']. " WHERE type = 'PT'";
	try
	{
	...
	//Staff (Ordered by workload amount for use later)
	...
	$query_rsProjPref           = "SELECT * FROM " . $TABLES['staff_pref_part_time'] . " WHERE (prefer LIKE 'SCE%' OR prefer LIKE 'SCSE%') AND archive=0 ORDER BY choice ASC";
	
	//$query_rsAreaPref           = "SELECT * FROM " . $TABLES['staff_pref_part_time'] . " WHERE prefer not LIKE 'SCE%' ORDER BY choice ASC";
	
	$query_rsAreaPref 	= "SELECT * FROM ".$TABLES['staff_pref_part_time']." as sp INNER JOIN ". $TABLES['interest_area'] ." as ia ON sp.prefer= ia.key  AND  archive =0 ORDER BY choice ASC";
	
	$query_rsExaminableProject  = "SELECT p3.project_id as pno, p2.staff_id as staffid, p3.examine_year as exam_year, p3.examine_sem as exam_sem, p1.title as ptitle, p1.Area1 as parea1 , p1.Area2 as parea2 , p1.Area3 as parea3 , p1.Area4 as parea4 , p1.Area5 as parea5 FROM " . $TABLES['fea_projects_part_time'] . " as p3 LEFT JOIN " . $TABLES['fyp_assign_part_time'] . " as p2 ON p3.project_id=p2.project_id  LEFT JOIN " . $TABLES['fyp'] . " as p1 ON p2.project_id=p1.project_id WHERE p2.complete = 0 and p3.examine_year = " . $examYearValue . " and p3.examine_sem = " . $examSemValue;
...
try {
	...
	$WORKLOAD_TOTALPROJECTS = $examinableProject->rowCount();
    ...
    $updateQuery = sprintf("INSERT INTO %s (`project_id`, `examiner_id`, `day`, `slot`, `room`) VALUES %s ON DUPLICATE KEY UPDATE `examiner_id`=VALUES(`examiner_id`), `day`=NULL, `slot`=NULL, `room`=NULL", $TABLES['allocation_result_part_time'], implode(",", $values));
...
function Algorithm_Random($staffList, $examinableProjectList, $ss_KeyList, $WORKLOAD_PER_PROJECT_EXAMINED, $WORKLOAD_TOTALPROJECTS){
    ...
    while(sizeof($WorkingStaffList) > 0){
    ...
    try{
        ...
        $Contents = $String01;
        $file = "submit_allocate_examiner_result_part_time.txt";
