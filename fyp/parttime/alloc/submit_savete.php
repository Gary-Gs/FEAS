...
<?php
	...
	try
	{
		$conn_db_ntu->exec("DELETE FROM ".$TABLES['fea_settings_availability_part_time']);
	}
	...
	while(isset($_REQUEST['staff_'.$i]) ||
		...
		if(empty($staff) || empty($day) || empty($timestart) || empty($timeend)) {
			//echo "Has Empty Fields";
					...
					$stmt = $conn_db_ntu->prepare("SELECT * FROM  ". $TABLES['fea_settings_availability_part_time'] . " WHERE staff_id = ? and day = ? and time_start = ? and time_end = ?");
				...
				if (sizeof($existRecords) <= 0)	//Has no duplicates
				{
					...
					$stmt = $conn_db_ntu->prepare("INSERT INTO ". $TABLES['fea_settings_availability_part_time'] . " (staff_id, day, time_start, time_end) VALUES (?, ?, ?, ?)");
					$stmt->bindParam(1, $staff);
					$stmt->bindParam(2, $day);
					$stmt->bindParam(3, $timestart);
					$stmt->bindParam(4, $timeend);
